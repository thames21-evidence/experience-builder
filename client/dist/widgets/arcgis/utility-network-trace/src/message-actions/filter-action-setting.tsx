...
const DSSelectorTypes = Immutable([DataSourceTypes.FeatureLayer])
  ...
  getInitConfig = () => {
    ...
    if (!this.props.config.messageUseDataSource) {
      if (
        ...
        if (
          dsJson &&
          (dsJson.type === DataSourceTypes.WebMap ||
            dsJson.type === DataSourceTypes.WebScene)
        ) {
          messageUseDataSource = null
        } else {
        ...
        {((!!config.messageUseDataSource && !!config.actionUseDataSource) ||
          ...
          <Fragment>
            <SettingSection title={this.formatMessage('conditions')}>
              ...
              {!isExtendChange && (
                <SettingRow tag='label' label={this.formatMessage('relateMessage')}>
                  <Switch

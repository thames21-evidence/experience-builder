...
const getPolygonSymbol = (graphic: __esri.Graphic, color: string): __esri.Graphic => {
  ...
  const graphics = new Graphic({
    geometry: graphic.geometry,
    symbol: symbolPolygon as unknown as __esri.SymbolUnion,
    attributes: graphic.attributes
  })
  return graphics
}
 ...
 *
 * @param symbol - The symbol to be used for the buffer.
 *                  If the symbol is invalid or cannot be parsed using `jsonUtils.fromJSON`, a default polygon symbol
 *                  will be created using the `getSketchSymbol` function.
 * @param symbolColor - Optional. The color string to be used for the buffer symbol. If not provided, a default color is used.
 * @returns - Returns the symbol for the buffer using the provided or default symbol color.
 */
export const getBufferSymbol = (symbol: any, symbolColor?: string): __esri.Symbol => {
  const parsedSymbol = symbol ? jsonUtils.fromJSON(symbol) : null
  if (parsedSymbol) {
    return parsedSymbol
  } else {
    //buffer is always a polygon hence pass polygon geometry type
    const bufferSymbol: any = getSketchSymbol('polygon', symbolColor)
    if (bufferSymbol) {

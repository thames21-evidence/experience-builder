...
export default class AoiTool extends React.PureComponent<Props, State> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      showInfoIcon: false,
      widgetClosed: this.props.widgetClosed
    }
  }
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (prevProps.highlightColor !== this.props?.highlightColor) {
      ...
      if (this.props.bufferLayer?.graphics?.length > 0) {
        const bufferSymbol = getBufferSymbol(this.props.bufferSymbol, this.props.highlightColor)
        if (bufferSymbol) {
    ...
    //check if configured buffer symbol color is changed
    if (prevProps.bufferSymbol !== this.props?.bufferSymbol) {
      if (this.props.bufferLayer?.graphics?.length > 0) {
        const bufferSymbol = getBufferSymbol(this.props.bufferSymbol, this.props.highlightColor)
        if (bufferSymbol) {
          this.props.bufferLayer.graphics.forEach((graphic) => {
    ...
    //update the widget state opened/closed
      ...
      geometryUtils.projectToSpatialReference([bufferGeometry],
        ...
        if (bufferGeometryInMapSr?.length > 0) {
          const bufferGraphic = new Graphic({
            ...
            symbol: getBufferSymbol(this.props.bufferSymbol, this.props.highlightColor) as __esri.SymbolUnion
          })
          if (bufferGraphic) {
            this.props.bufferLayer.removeAll()
            ...
            this.props.jimuMapView?.view.goTo({
              center: bufferGraphic.geometry.type === 'point' ? bufferGraphic.geometry : bufferGraphic.geometry.extent.clone().expand(1.5)
            })
          }
          this.setState({
      ...
      <div className='pt-2 px-2' css={getAoiToolStyle(this.props?.theme, showSketchTools)}>
        <div className={searchCurrentExtent && this.props.headingLabel ? 'mb-2' : 'hidden'}>
          <Label className={'headingLabelStyle mb-0'}>
            ...
            {false && searchCurrentExtent && this.state.showInfoIcon &&//eslint-disable-line no-constant-binary-expression
        ...
        <div className='main-row w-100'>
          <div className={locateIncidentStyles}>
            <LocateIncident
              ...
              activateToolOnWidgetLoad={this.props.activateToolOnWidgetLoad}

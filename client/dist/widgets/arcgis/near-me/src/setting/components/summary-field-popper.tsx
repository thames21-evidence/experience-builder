...
import type { CurrentLayer, LayersInfo, SelectedExpressionInfo, SummaryExpressionFieldInfo, SummaryFieldsInfo } from '../../config'
...
export default class SummaryFieldPopper extends React.PureComponent<Props, State> {
  constructor (props) {
    super(props)
    const useDs = this.props.layersInfos.filter((layer) => layer.useDataSource.dataSourceId === this.props.currentLayerDsId.layerDsId)
    this.state = {
      useDataSource: [useDs[0].useDataSource],

...
import { formatMessage, getConfigValue, lrsDefaultMessages, updateConfig, waitTime } from 'widgets/shared-code/lrs'
...
export function DisplaySettings(props: Props) {
  ...
  const hideMeasures = useConfigValue('hideMeasures', false)
  ...
  const handleHideMeasuresChange = (e: any, checked: boolean) => {
    updateConfig(widgetId, config, 'hideMeasures', checked, activeMapViewId, onSettingChange)
  }
    ...
    <React.Fragment>
      <SettingSection className="px-4 ">
        ...
        >
          ...
          <SettingRow tag="label" label={formatMessage(intl, 'hideMeasures')}>
            <Tooltip title={getI18nMessage('hideMeasureTooltip')}>
              <Switch checked={hideMeasures} onChange={handleHideMeasuresChange} />
            </Tooltip>
          </SettingRow>

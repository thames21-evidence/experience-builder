...
import { type LrsLayer, LrsLayerType, SearchMethod, getConfigValue, isDefined, isPointEvent, lrsDefaultMessages, updateConfig } from 'widgets/shared-code/lrs'
...
export function DefaultSettings(props: Props) {
  ...
  const defaultMethod = useConfigValue('defaultMethod', SearchMethod.Measure)
  ...
  const getDefaultEvent = (): string => {
    if (defaultEvent && defaultEvent.index >= 0) {
      ...
      const firstLineLayer = lrsLayers.find((layer) => isPointEvent(Immutable(layer)))
  ...
  const getDefaultNetworkConfig = (): string => {
    if (defaultNetwork && defaultNetwork.index !== -1) {
      ...
      const firstNetworkLayer = lrsLayers.find((layer) => layer.layerType === LrsLayerType.Network && !layer.networkInfo?.isDerived)
  ...
  const handleDefaultMethodChanged = (value: SearchMethod) => {
    ...
    <React.Fragment>
      <SettingSection className="px-4">
        ...
        >
          ...
          <SettingRow flow="wrap" label={getI18nMessage('defaultMethod')}>
            <Select
              aria-label={getI18nMessage('defaultMethod')}
              className="w-100"
              size="sm"
              value={defaultMethod}
              onChange={(e) => {
                handleDefaultMethodChanged(e.target.value)
              }}
            >

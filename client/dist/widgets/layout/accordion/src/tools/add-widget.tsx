...
import { isLayoutItemAccepted, addItemToAccordion, widgetToolbarStateChange } from './utils'
import { isInController } from '../runtime/util'

export default class AddWidget implements extensionSpec.ContextToolExtension {
  index = 4
  id = 'accordion-add-widget'
  name = 'accordion-add-widget'
  widgetId: string
  ...
  visible (props: LayoutContextToolProps) {
    ...
    if (window.jimuConfig.isBuilder) {
      appConfig = getAppStore().getState().appStateInBuilder.appConfig
    } else {
      appConfig = getAppStore().getState().appConfig
    }
    const isControllerGroup = isInController(layoutId, appConfig)
    return !isControllerGroup
  }
  ...
  onClick (props: LayoutContextToolProps) {
    ...
    if (browserSizeMode === BrowserSizeMode.Small) {
      ...
      const onClose = () => {
        ...
        widgetToolbarStateChange(widgetId, ['accordion-add-widget'])
      }
      appBuilderSync.publishSidePanelToApp({
        ...
        onSelect: (item: LayoutItemConstructorProps) => {
          addItemToAccordion(widgetId, item)
        },
        onClose
      })
    }
